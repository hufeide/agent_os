...
try:
        ...
        for i, task in enumerate(detailed_status['tasks'], 1):
            ...
            if task.get('result'):
                ...
                if result.get('findings'):
                    ...
                    print(f"    ✓ 有研究发现: {findings[:200]}...")
                    has_results = True
                if result.get('insights'):
                    insights = result['insights']
                    print(f"    ✓ 有分析洞察: {insights[:200]}...")
                    has_results = True
                if result.get('output'):
                    output = result['output']
                    print(f"    ✓ 有写作输出: {output[:200]}...")
                    has_results = True
                
                # 检查关键点
                if result.get('key_points'):
                    key_points = result['key_points']
                    print(f"    ✓ 有关键点: {len(key_points)}个")
                    has_results = True
                
                # 检查数据源
                if result.get('data_sources'):
                    data_sources = result['data_sources']
                    print(f"    ✓ 有数据源: {len(data_sources)}个")
                    has_results = True
                
                # 检查执行步骤
                if result.get('steps'):
                    steps = result['steps']
                    print(f"    ✓ 有执行步骤: {len(steps)}个")
                    has_results = True
            else:
                print(f"    ✗ 没有执行结果")
    ...
    if found_task:
        ...
        print(f"  ✓ 完成任务数: {found_task.get('completed_count', 0)}")
        
        # 检查是否有最新结果
        if found_task.get('latest_result'):
            latest_result = found_task['latest_result']
            print(f"  ✓ 有最新结果")
            print(f"    摘要: {latest_result.get('summary', '')[:200]}...")
        ...
        if found_task.get('completed_results'):
            completed_results = found_task['completed_results']
            print(f"  ✓ 有完成结果列表，数量: {len(completed_results)}")
            for i, completed_result in enumerate(completed_results, 1):
                print(f"    结果 {i}: {completed_result.get('task_name')} - {completed_result.get('result', {}).get('summary', '')[:100]}...")
            has_results = True
        else:
            print(f"  ✗ 没有完成结果列表")
    else:
